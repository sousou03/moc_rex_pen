	  ...
	  _createClass(Dom, [{
	    ...
	    value: function update(delta) {
	      ...
	      while (phi >= this.splineX[j]) {
	        i = j++;
	      }var rho = this.interpolation(this.splineY[i], this.splineY[j], (phi - this.splineX[i]) / (this.splineX[j] - this.splineX[i]));
	      phi *= Math.PI * 2;
	    ...
	    key: 'interpolation',
	    value: function interpolation(a, b, t) {
	      return (1 - Math.cos(Math.PI * t)) / 2 * (b - a) + a;
	    }
	  }, {
	    key: 'createPoisson',
	    value: function createPoisson() {
	      // domain is the set of points which are still available to pick from
	      // D = union{ [d_i, d_i+1] | i is even }
	      var domain = [this.radius, 1 - this.radius],
	          measure = 1 - this.radius2,
	          spline = [0, 1];
	      while (measure) {
	        var dart = measure * Math.random(),
	            i,
	            l,
	            interval,
	            a,
